...
# -------- NEW: try to import pygame for the Pong game --------
try:
    import pygame
    ...
    print("⚠️ pygame not installed. Pong game will be disabled.")

# remove external oculy_pong import – we'll define update_eye_action locally
...
def action_decode(action):
    if action == '0':
        ...
        return f"config,{input('MAC or DEVICE_ID: ')}"
    elif action == '2':
        ...
        return f"enable,{input('MAC or DEVICE_ID: ')}"
    elif action == '4':
        return f"disable,{input('MAC or DEVICE_ID: ')}"
    elif action == '5':
        return 'start'
    elif action == '6':
        device = input('MAC or DEVICE_ID: ')
...
# ---------- NEW: Simple Pong implementation + update_eye_action ----------
class PongGame:
    """
    Minimal Pong game controlled by eye-action labels:
      - "left"  -> move paddle left
      - "right" -> move paddle right
      - "neutral"/other -> stop paddle
    """

    def __init__(self, width=800, height=400):
        if not PYGAME_AVAILABLE:
            raise RuntimeError("pygame not available, cannot start Pong game.")

        pygame.init()
        pygame.display.set_caption("Oculy Pong")
        self.width = width
        self.height = height
        ...
        # Action from classifier: -1 (left), 0 (neutral), +1 (right)
        self.action_dir = 0
        self.action_lock = threading.Lock()

    def set_action_from_label(self, label: str):
        """
        Called from update_eye_action; maps label to a movement direction.
        """
    ...
    def _update_paddle(self):
        with self.action_lock:
            dx = self.action_dir * self.paddle_speed
        self.paddle_x += dx
        # clamp to screen
        if self.paddle_x < 0:
            self.paddle_x = 0
    ...
    def _update_ball(self):
        ...
        # Paddle collision
        if (self.paddle_y - self.ball_radius <= self.ball_y <= self.paddle_y + self.paddle_height
        ...
        if self.ball_y - self.ball_radius > self.height:
            ...
            self.ball_x = self.width // 2
            self.ball_y = self.height // 2
            self.ball_vx = 4 * np.sign(self.ball_vx or 1)
            self.ball_vy = -4
    ...
    def run(self):
        """Main game loop. Intended to run in its own thread."""
        ...
        pygame.quit()
        print("🎮 Pong game stopped.")


# Global game instance + update_eye_action hook
pong_game: Optional[PongGame] = None

def start_pong_game_in_thread():
    global pong_game
    if not PYGAME_AVAILABLE:
        print("⚠️ Cannot start Pong game: pygame not installed.")
        return
    if pong_game is not None:
        return
    pong_game = PongGame()
    t = threading.Thread(target=pong_game.run, daemon=True)
    t.start()


def update_eye_action(label: str):
    """
    This replaces the old import from oculy_pong.
    Called from handle_prediction. Forwards the label to the Pong game.
    """
    global pong_game
    if pong_game is None:
        # Lazy-start the game when the first action arrives
        try:
            start_pong_game_in_thread()
        except Exception as e:
            print(f"⚠️ Failed to start Pong game: {e}")
            return

    if pong_game is not None:
        pong_game.set_action_from_label(label)
...
class LiveClassifier:
    ...
    def update_metadata_from_packet(self, packet: dict):
        ...
        if isinstance(rd, dict):
            ...
            if filtered := {k: v for k, v in candidates.items()}:
                candidates = filtered
        if not candidates:
            return
...
class TCPClient:
    ...
    def addMsgToSend(self, data):
        if "start" in data:
            self.setIsAcquiring(True)
        elif "stop" in data:
            self.setIsAcquiring(False)
